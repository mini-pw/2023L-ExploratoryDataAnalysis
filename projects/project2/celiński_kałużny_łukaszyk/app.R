...
wykresowa <- read.csv("ramki_danych/finalna_git.csv")
wykresowa <- wykresowa %>% 
  mutate(category = factor(category, levels = unique(category))) %>% 
  mutate(winner = as.logical(winner))

x1 <- read.csv("ramki_danych/aktorzy_wygrani2.csv")
  ...
  output$wykres_a2 <- renderPlotly(
    {
      ...
      a2 <- ggplot(filter(y1, sex == "k"),aes(oscar_age)) + 
        ...
        scale_y_continuous(expand = c(0,0)) +
        scale_x_continuous(expand = c(0,0)) +
        labs(x = "wiek", y = "liczba aktorów", title = "Wiek aktorów", caption = "czerwony kolor - kobiety, niebieski - mężczyźni")
  ...
  output$wykres_a3 <- renderPlotly(
    {
      ...
      a3 <- ggplot(filter(y1, sex == "m"),aes(oscar_age)) + 
        ...
        scale_y_continuous(expand = c(0,0)) +
        scale_x_continuous(expand = c(0,0)) +
        labs(x = "wiek", y = "liczba aktorek", title = "Wiek aktorek", caption = "żółty kolor - role pierwszoplanowe, zielony - drugoplanowe")
