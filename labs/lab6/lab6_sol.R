...
## Zadanie 1
# Przyjżyjmy się bliżej Kalifornii, Oregon i Waszyngtonowi. Przygotuj mapę zawierającą te stany.
...
#  Przyjżyjmy się teraz bliżej Kalifornii
...
pop_and_ares <- read.csv("pop_and_area.csv")

ca <- ca_county %>% left_join(pop_and_ares, by = c("subregion" = "Country"))
...
ca_base + 
  ...
  theme_bw() +
  theme_nothing +
  labs(title = "Gęstość zaludnienia stanu Kalifornia (mile kwadratowe)",
       subtitle = "Rok 2021",
       fill = "Gęstość")
...
ca$people_per_km <- ca$people_per_sq_mile*0.386

ca_density <- ca_base + 
  geom_polygon(data = ca, aes(fill = people_per_km), color = "white") +
  geom_polygon(color = "black", fill = NA) +
  theme_bw() +
  theme_nothing +
  labs(title = "Gęstość zaludnienia stanu Kalifornia",
       subtitle = "Rok 2021",
       fill = "Gęstość") +
...
ca_density + coord_fixed(xlim = c(-123, -121.0),  ylim = c(36, 38), ratio = 1.3)
